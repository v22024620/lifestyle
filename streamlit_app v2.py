...
def render_member_academy_section(
    ...
    render_metric_card(col3, "회원 세그먼트", segment, "Apple Fitness+ 톤")
...
def render_operator_section_selector(default_section: Optional[str] = None) -> str:
    desc_map = {
        ...
        "Partner Safety Desk": "결제·정산 리스크를 Apple Fitness+ 톤으로 안내합니다.",
    }
...
def get_backend_health_snapshot() -> Dict[str, Dict[str, str]]:
    orchestrator = load_orchestrator()
    ...
    def _status(latency: Optional[int], connected: bool) -> Dict[str, str]:
        if latency is None:
            base = {"status": "알 수 없음", "detail": "헬스 데이터를 받아오는 중"}
        elif latency < 200:
            base = {"status": "안정", "detail": f"응답 {latency}ms"}
        elif latency < 400:
            base = {"status": "주의", "detail": f"응답 {latency}ms"}
        else:
            base = {"status": "집중 케어", "detail": f"지연 {latency}ms"}
        suffix = "Connected" if connected else "Stub"
        base["detail"] = f"{base['detail']} · {suffix}"
        return base
    return {
        "VectorDB": _status(latency_vector, vector_connected),
        "GraphDB": _status(latency_graph, graph_connected),
        "Neo4j": _status(latency_neo4j, neo4j_connected),
    }
...
def render_operator_network_section(model: Dict[str, Any]) -> None:
    ...
    for col, label in zip((col_a, col_b, col_c), ("VectorDB", "GraphDB", "Neo4j")):
        status = health.get(label, {"status": "알 수 없음", "detail": "헬스 데이터 준비중"})
        html_block = ("<div class='lop-card'>"
            f"<div class='lop-label-muted'>{label}</div>"
            f"<div class='lop-metric'>{status['status']}</div>"
            f"<div class='lop-card-note'>{status['detail']}</div>"
            "</div>")
...
def render_operator_view(bundle: Optional[Dict[str, Any]], *, studio_id: str, query_meta: Dict[str, str]) -> None:
    ...
    render_header(
        ...
        subtitle="멀티 에이전트 신호와 네트워크 상태를 Apple Fitness+ 톤으로 정리했습니다.",
...
def render_studio_view(bundle: Optional[Dict[str, Any]], *, studio_id: str, query_meta: Dict[str, str]) -> None:
    ...
    render_header(
        ...
        subtitle="Apple Fitness+ 톤으로 오늘의 운영 신호를 정리했습니다.",
        badge="실행 요약",
    )
...
def render_member_view(
    ...
    *,
    ...
    render_text_card("오늘의 톤", hero_copy, footer="Apple Fitness+ 스타일")
...
def main() -> None:
    ...
    if persona == "회원":
        ...
        member_name = st.sidebar.text_input("회원 이름", key="sidebar_member_name", autocomplete="off")
    else:
